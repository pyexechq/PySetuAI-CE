...
- [x] Create migration `053_policy_bundle_mcp_scope.py` adding nullable JSONB `mcp_scope` to `policy_bundles`.
- [x] Add `mcp_scope` to `PolicyBundle` model and Pydantic schemas (`McpScopeConfig` with `mode`, `entries`).
- [x] Update bundle CRUD in `governance.py` API to accept/return `mcp_scope`.
- [x] Run `make migrate` and confirm head `053_policy_bundle_mcp_scope`.
...
- [x] Create `backend/app/services/mcp_access_service.py` with:
...
- [x] Wire `is_tool_denied` from `mcp_security_service`.
- [x] Unit tests in `test_mcp_access.py` for allowlist, deny, compat `all` mode.
...
- [x] In `_handle_mcp_multiplex` (`gateway.py`):
...
- [x] Tests in `test_mcp_multiplex_audit.py` with mocked MCP upstream.
...
- [x] In `gateway_service.py` chat flow (~line 512): replace inline server list logic with `mcp_access_service` (keep dynamic tools downstream).
- [x] If model invokes tools via chat completion tool_calls, audit + inspect on same path where tool execution occurs (if applicable in current code).
...
- [x] In `llm_router.select_model`: for each active rule, if `routing_rule_client_keys` rows exist, rule matches only when `ctx.client_api_key_id` is in assigned set; JWT requests skip key-scoped rules (or only match when no assignments).
- [x] Add `test_llm_router_key_binding.py`.
...
- [x] Update `docs/testing/test-plan.md` MCP-005, MCP-009 to Pass when implemented.
- [x] Update `docs/testing/release-readiness.md` MCP Governance row.
- [x] Mark BL-098–BL-103 done in backlog.
