...
const EMPTY: ApiIntegrationSettings = {
  ...
  env_fallback_note:
    "Environment variables (.env / Docker) are used when tenant settings are empty. Tenant settings here take priority.",
};

export function TenantLlmProviderSettings() {
  ...
  return (
    <Card id="tenant-llm-defaults" className="border-border/60 bg-card/50">
      <CardHeader>
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div>
            <CardTitle className="flex items-center gap-2">
              <KeyRound className="h-5 w-5" />
              Tenant default LLM providers
            </CardTitle>
            <CardDescription>
              Fallback credentials for gateway traffic when the model registry does not supply a key. AI Assist can
              inherit these keys when no dedicated assist key is set.
            </CardDescription>
          </div>
          <Badge variant={settings.active_upstream === "mock" ? "warning" : "success"}>
            Gateway active: {settings.active_upstream}
          </Badge>
        </div>
      </CardHeader>
      <TenantLlmProviderForm
...
function TenantLlmProviderForm({
  settings,
  token,
  canEdit,
  queryClient,
  ...
  return (
    <CardContent className="space-y-6">
      <p className="rounded-md border border-border/60 bg-muted/20 p-3 text-xs text-muted-foreground">
        ...
        {settings.secrets_backend === "vault" && settings.vault_auth_method
          ? ` (auth: ${settings.vault_auth_method})`
          : null}
        . For routed models, prefer credentials on{" "}
        <Link href="/llm-router" className="text-primary underline">LLM Router → Model registry</Link>.
      </p>
      ...
      <div className="space-y-2">
        ...
        />
        <p className="text-xs text-muted-foreground">
          Gateway priority: OpenAI → Gemini → Ollama. Request a model containing &quot;gemini&quot; to force Gemini.
        </p>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">Local Ollama when no cloud API key is configured.</p>
        </div>
        <div className="space-y-2">
